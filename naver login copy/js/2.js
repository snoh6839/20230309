...
function isOldIE() {
    try {
        ...
        if (agent.indexOf("MSIE 8") != -1 || agent.indexOf("MSIE 7") != -1) {
            login_chk.attachEvent("onclick", function () {
                msieblur(login_chk);
            });
        }
...
function confirmAbroadContactSubmit() {
    ...
    if (selectedTab.value == "email") {} else {
...
function addNclicksEvent(nclickId) {
    if (isObjExist(nclickId)) {
        ...
        if (targetElement.addEventListener) {
            ...
            targetElement.attachEvent("onclick", function (e) {
                nclk_v2(e, nclickId);
            });
        }
    }
...
if (isObjExist('locale_switch')) {
    ...
    if (targetElement.addEventListener) {
        ...
        targetElement.addEventListener("change", function (e) {
            var nclicksMapJson = {
                'ko_KR': pageDirective + '.lankr',
                'en_US': pageDirective + '.lanen',
                'zh-Hans_CN': pageDirective + '.lancn',
                'zh-Hant_TW': pageDirective + '.lantw'
            };
